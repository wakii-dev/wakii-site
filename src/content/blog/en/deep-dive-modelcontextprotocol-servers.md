...
- **DIRECTION** — deprecation with an address: all 13 archived servers name their replacement (Brave Search → Brave's official server; Slack → Zencoder). Wakii's skills catalog (20 skills at time of writing) will need retirement as it grows — a pattern worth adopting: keep the entry, point to the successor, never delete silently.
