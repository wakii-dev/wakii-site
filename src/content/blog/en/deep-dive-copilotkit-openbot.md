...
- **ADOPT** — refusals must name their rule: OpenBot's `ActionRefusedError` carries the exact rule that blocked an action so the UI can show it. Wakii already does this for gate guard codes (FI-341: duplicate resolves and closed gates are rejected with explicit error codes) — OpenBot confirms the pattern and suggests extending it: every agent refusal, including outside gates (a denied tool, a read-only file), should return a rule identifier instead of a generic message.
