# TabbyML/tabby — research digest (batch-3, matrix #45)

> Skeleton FI-383 SF-1 — SF sở hữu ĐIỀN các TODO dưới đây trước khi viết bài
> (research thật: đọc README + code, không dùng trí nhớ). Tên file = slug bài.
> KHÔNG bịa số không probe; mọi số third-party cite kèm ngày lấy
> (claims-registry §Third-party claims — batch-3).

- repo: TabbyML/tabby
- facet: editors
- stars @ 2026-09-08: 33869
- license (GitHub API 2026-09-08): 2026-06-30
- probe-date: 2026-09-08 (`bash scripts/probe-repos.sh`)

## TODO — điền khi research (SF sở hữu)

- [ ] README notes — repo làm gì, cho ai, điểm khác biệt cốt lõi
- [ ] Architecture — cấu trúc code, cơ chế đáng học (đọc code thật, trích kèm link commit/tree)
- [ ] Releases — cadence + release gần nhất (gh release list / API, kèm ngày probe)
- [ ] Wakii grading — ADOPT / DIRECTION / WATCH / N/A, mỗi grade ≥1 lý do (style-guide §8)
