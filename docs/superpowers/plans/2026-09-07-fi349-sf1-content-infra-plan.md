...
- [ ] 1. schema-extension-optional-defaults: `src/content.config.ts` — blog schema thêm `heroImage: z.string().optional()` + `author: z.string().default('Wakii team')`; KHÔNG đụng category enum, docs collection.
- [ ] 2. category-label-map-i18n-shared: `src/i18n/categories.ts` — `CATEGORY_LABELS` EN/VI (tutorial→tutorial/hướng dẫn, tech→tech notes/kỹ thuật, build-log→build log/nhật ký xây dựng — strings pre-drafted spec SF-3); CHỈ tạo map, không adopt vào pages.
- [ ] 3. readingtime-util-wpm-en200-vi160: `src/utils/blog.ts` — `readingTime(body, locale)`: đếm từ trên markdown body, WPM EN 200 / VI 160, làm tròn lên, min 1 phút.
- [ ] 4. related-util-fallback-chain: `src/utils/blog.ts` — `relatedPosts(current, all, max=3)`: priority tiers same-category → shared-tags (≥1 tag chung) → latest-same-locale (fill-chain, dedupe, cap 3); draft:false + same-locale BẮT BUỘC mọi tier.
- [ ] 5. utils-assertion-script: `scripts/check-blog-utils.mjs` — node assert utils trên 10 posts thật: readingTime(vi) ≥ readingTime(en) cùng post + ≥ 1 + ceil-check; related('building-wakii…') = 3 items latest-same-locale (không chứa current, same locale); author default path. npm script `check:blog-utils` + chain vào `build`.
