...
- [ ] 3. catalog-route-en — `src/pages/skills.astro`: Base + Nav/Footer sẵn; hero
      ...
      `<code>`, hero-stats 3 ô derive: count/categories/MIT); tokens theo mapping
      plan-hand-off (token có sẵn thắng literal; card radius `--radius-cell`).
...
- [ ] 5. category-grouping-theo-taxonomy-SF-1 — filter `public === true` (13),
      ...
      copy EN/VI theo hand-off §3; platform (7) không render.
...
- [ ] 9. seo-meta-hreflang-route-mới — title `skills — wakii` / VI tương ứng
      ...
      regression hreflang 1 trang docs cũ.
